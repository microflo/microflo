...
var sendCommands = function(transport, data, callback) {
...
var uploadGraph = function(transport, data, graph, receiveHandler, onlyRegisterHandler) {
    var handler = receiveHandler ? receiveHandler : printReceived;

    if (onlyRegisterHandler) {
        // HACK: because the onData handling is not resable outside
        throw new Error('Use the DeviceCommunication API instead!');
    }
    ...
    var comm = new devicecommunication.DeviceCommunication(transport, graph, componentLib);
    comm.on('response', function() {
        handler.apply(this, arguments);
    });
    comm.open(function() {
        comm.sendCommands(data, function(err) {
            comm.close(function(err) {
...
var handleRuntimeCommand = function(command, payload, connection) {

    if (command == "getruntime") {
        var caps = ["protocol:graph", "protocol:network", "protocol:component" ];
        var r = { type: "microflo", version: "0.4", capabilities: caps}
        connection.send({ protocol: "runtime", command: "runtime", payload: r});
    } else {
...
var handleComponentCommand = function(command, payload, connection) {
    if (command == "list") {
        ...
        componentLib.getComponentSource(payload.name, function(err, source) {
            var r = { name: payload.name, language: "c++", code: source };
            connection.send({protocol: "component", command: "source", payload: r});
        });
